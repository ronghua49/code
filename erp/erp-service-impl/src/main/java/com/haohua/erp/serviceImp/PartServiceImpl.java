    ...
    public void receiveParts(Integer orderId) {
       ...
       //判断所领取的配件数量是否大于库存
         for(FixTransParts fixTransParts :fixTransPartsList){
            ...
            if (parts.getNum()<fixTransParts.getPartsNum()){
                throw  new ServiceException("编号为 "+parts.getPartsNo()+"的"+parts.getPartsName()+"库存不足!");
            }else{
                //领取配件
                parts.setNum(parts.getNum()-fixTransParts.getPartsNum());
                ...
                partsStream.setBeforeNum(parts.getNum());
                partsStream.setNum(fixTransParts.getPartsNum());
                partsStream.setAfterNum(parts.getNum()-fixTransParts.getPartsNum());
